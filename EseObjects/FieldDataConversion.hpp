...
//convert to specific using type specified in .NET generic
generic <class T> T from_memblock_generic(void *buff, ulong max, JET_COLTYP coltyp, ushort cp)
{
	if(T::typeid == Boolean::typeid)
		return safe_cast<T>(from_memblock<Boolean>(buff, max, coltyp, cp));
	else if(T::typeid == Byte::typeid)
		return safe_cast<T>(from_memblock<Byte>(buff, max, coltyp, cp));
	else if(T::typeid == SByte::typeid)
		return safe_cast<T>(from_memblock<SByte>(buff, max, coltyp, cp));
	else if(T::typeid == Char::typeid)
		return safe_cast<T>(from_memblock<Char>(buff, max, coltyp, cp));
	else if(T::typeid == Single::typeid)
		return safe_cast<T>(from_memblock<Single>(buff, max, coltyp, cp));
	else if(T::typeid == Double::typeid)
		return safe_cast<T>(from_memblock<Double>(buff, max, coltyp, cp));
	else if(T::typeid == Int16::typeid)
		return safe_cast<T>(from_memblock<Int16>(buff, max, coltyp, cp));
	else if(T::typeid == Int32::typeid)
		return safe_cast<T>(from_memblock<Int32>(buff, max, coltyp, cp));
	else if(T::typeid == Int64::typeid)
		return safe_cast<T>(from_memblock<Int64>(buff, max, coltyp, cp));
	else if(T::typeid == UInt16::typeid)
		return safe_cast<T>(from_memblock<UInt16>(buff, max, coltyp, cp));
	else if(T::typeid == UInt32::typeid)
		return safe_cast<T>(from_memblock<UInt32>(buff, max, coltyp, cp));
	else if(T::typeid == UInt64::typeid)
		return safe_cast<T>(from_memblock<UInt64>(buff, max, coltyp, cp));
	else if(T::typeid == Guid::typeid)
		return safe_cast<T>(from_memblock<Guid>(buff, max, coltyp, cp));
	else if(T::typeid == String::typeid)
		return safe_cast<T>(from_memblock<String ^>(buff, max, coltyp, cp));
	else if(T::typeid == array<uchar>::typeid)
		return safe_cast<T>(from_memblock<array<uchar> ^>(buff, max, coltyp, cp));
	else if(T::typeid == DateTime::typeid)
		return safe_cast<T>(from_memblock<DateTime>(buff, max, coltyp, cp));

	throw gcnew InvalidOperationException("Cannot convert column data to specified type: " + T::typeid->ToString());
}
